 ...
 */
public class UpdateLikeordislike extends SqlUpdate {
    private static final String SQL_UPDATE="UPDATE rating_personal_table " +
            "SET user_creator=(SELECT id FROM user_table WHERE username=:user_creator), idea_link=:idea_link,likeordislike=:likeordislike " +
            "WHERE user_creator=(SELECT id FROM user_table WHERE username=:user_creator) AND idea_link=:idea_link";

    public UpdateLikeordislike(DataSource dataSource){
